...
D3D12_RASTERIZER_DESC CShader::CreateRasterizerState()
{
	...
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID; // 내부를 스캔라인을 통해 그린다.
	//D3D12_FILL_MODE_WIREFRAME은 프리미티브(삼각형)의 내부를 칠하지 않고 변(Edge)만 그린다.
	//d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_WIREFRAME;

	d3dRasterizerDesc.CullMode = D3D12_CULL_MODE_NONE; // 컬링 모드를 사용하지 않는다.
	//d3dRasterizerDesc.CullMode = D3D12_CULL_MODE_BACK; // 뒷면을 컬링한다.
	//d3dRasterizerDesc.CullMode = D3D12_CULL_MODE_FRONT; // 앞면을 컬링한다.
